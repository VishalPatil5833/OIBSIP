...
class user
{
	...
	void tckt() 
	{
		...
		{	
			...
			{
				System.out.println("No Data Found in The chart");
			}
		}
		catch (Exception e)
		{
	...
	void chart(String pname[],int page[],String coach,int tnum,java.sql.Date dt2)
	{
		try
		{
			...
			r1.first();
			PreparedStatement st=c.prepareStatement("insert into chart (name,age,gender,seatno,coach,status,timestamp,dot,tnum) values(?,?,?,?,?,?,?,?,?)", Statement.RETURN_GENERATED_KEYS);
			st.executeUpdate();	
			   st.setString(2,pname[i]);
				...
				{
				   long pnr = generatedKeys.getLong(1);	
	...
	void enquiry() throws Exception
	{
		...
		System.out.println("****************************************************************************************************************************************************************************");
		System.out.println("     1         KASHI EXP       86            MUMBAI          VARANASI       300             250           200             180           2023-01-23    01:30:00      07:30:00");
		...
		while(r.next())
		{
		    ...
		    int seats = r.getInt(3);
		    String bp = r.getString(4);
		    String dp = r.getString(5);
		    int fAC = r.getInt(6);
		    int sAC = r.getInt(7);
		    int tAC = r.getInt(8);
		    int sc = r.getInt(9);
		    Date doj = r.getDate(10);
		    String dtime = r.getString(11);
		    String atime = r.getString(12);
		    int sno = 1;
			//tnum**tname**seats**bp***dp**fAC**sAC*tAC**sc*doj***dtime*atime**sno
		    setw(tnum, tname, seats, bp, dp, fAC, sAC, tAC, sc, doj, dtime, atime, sno);			
		...
		{	
			...
			while (true) {
			    try {
			        ...
			        System.out.print("Enter date of train's journey in (yyyy-MM-dd) format : ");
			        dt = i.next(); // Prompt user again for input
			    }
			}
...
class railway
{
	...
	void main_menu() throws Exception
	{
		...
		switch(ch)
		{
			...
			case 2: u_user_login();
				break;
			default:System.out.println("Made By Vishal Patil");
				   System.exit(0);
				break;
